...
/// @notice Ethereum mainnet representation of HYPE, minted against wHYPE locked in the
///         RateLimitedOFTAdapter on HyperEVM.
contract HYPEOFT is OFT, RateLimiter {
    ...
    ) OFT("HYPE", "HYPE", _lzEndpoint, _delegate) Ownable(_delegate) {
        _setRateLimits(_rateLimitConfigs);
    }

    /// @dev Matches wHYPE on HyperEVM. Above sharedDecimals(), so amounts are truncated to 6 decimal
